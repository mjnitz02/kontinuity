...
//  KontinuityCore
//
//  The single boundary between the app and the server. The transports are split
//  deliberately — OPDS v2 for reading and sync, /api/v1 for browse and
//  read-status queries — and this protocol is where that split stays contained
//  instead of smearing across the feature code.
//
...
public protocol KomgaServing: Sendable {
    ...
    /// reading order, which is also the order downloads are enqueued in.
    ...
    /// instead of the network: a cover you've genuinely seen before shouldn't
    /// fail to load just because the server is unreachable right now. Each
    /// overload defaults to the other in a protocol extension, so
    ...
    /// width/height known before any image is fetched.
    ...
    /// entirely, the only way back to "never opened".
    ...
    /// fetches through ``pageImageData(at:)`` on a 403.
    ...
    /// app suspension — rather than the service's own foreground
...
/// bundled so `putProgression` stays under the line-count limit rather than
/// taking both as loose parameters.
...
/// bundled for the same reason as ``KomgaDevice``. `readDate` is the
/// timestamp of the page turn itself, not of the call — an outbox
...
public struct SeriesQuery: Sendable, Hashable {
    ...
    /// oneshots are invisible in most clients.
